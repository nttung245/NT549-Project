...
def main():
    ...
    print("\n🧪 Running Smoke Test: 5 episodes with random actions")
    print("=" * 70)
        ...
        while not done and step_count < 500:
            ...
            if step_count % 50 == 0 or done:
                ...
                print(f"  Step {step_count:>4}: Action={action}, RUL={rul:.1f}, "
                      f"Fuel={fuel:.1f}, Dist={dist:.0f}, Reward={reward:.0f}")

        final_event = info.get('event', 'TIMEOUT')
        print(f"  ✅ Episode {ep + 1} done: Steps={step_count}, "
              f"Total Reward={ep_reward:.0f}, Final Event={final_event}")
        if events:
            print(f"  Events: {events}")

    print("\n" + "=" * 70)
    print("🎉 Smoke test complete! Environment is stable.")
